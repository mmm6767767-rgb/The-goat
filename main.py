...
STATE_FILE = os.environ.get("STATE_FILE", "accounts_state.json")
...
# ---------------- persistent state (one-time onboarding per account) ----------------
...
class Account:
    def __init__(self, token: str, index: int):
        ...
        self.client = None
        self.state_key = token_key(token)
        self.initialized = self.state_key in STATE.get("initialized", [])
        self._make_client()
    ...
    def mark_initialized(self):
        if self.state_key not in STATE.setdefault("initialized", []):
            STATE["initialized"].append(self.state_key)
            save_state(STATE)
        self.initialized = True

    def should_be_online(self) -> bool:
        """
        Connect if onboarding is still pending (must run even on Friday/night).
        After onboarding: follow the schedule (night closed, Friday closed).
        """
        if not self.initialized:
            return True          # onboarding ALWAYS runs, any day, any hour
    ...
    async def run_quest_once(self, source: str = "slot"):
        ...
        if not doable:
            ...
            return
        print(f"[acc{self.index}] {len(doable)} uncompleted quests — completing ONE ({source})")
    ...
    # ---- startup: ONE-TIME onboarding per account (runs even on Friday/night) ----
    async def _on_ready(self):
        ...
        # ---- ONE-TIME onboarding: HypeSquad + bio + 1 quest, never again ----
        if self.initialized:
            print(f"[acc{self.index}] already onboarded previously — "
                  f"skipping HypeSquad/bio/onboarding quest (schedule only)")
        else:
            print(f"[acc{self.index}] FIRST RUN — onboarding (even on Friday/night): "
                  f"HypeSquad + bio + 1 quest")
            ...
            # complete ONE quest right now (one-time onboarding)
            await self.run_quest_once(source="onboarding")

            # mark done -> never runs again for this account (persists across restarts)
            self.mark_initialized()
            print(f"[acc{self.index}] onboarding COMPLETE — saved. Future runs = schedule only.")

            # onboarding done; if the schedule says closed right now (Friday/night),
            # disconnect immediately so the schedule rules apply from here on
            if not self.should_be_online():
                print(f"[acc{self.index}] schedule says closed (Friday/night) "
                      f"after onboarding — disconnecting now")
...
# ---------------- per-account runner (onboarding first, then schedule) ----------------
async def account_runner(acc: Account):
    while True:
        if not acc.should_be_online():
            await asyncio.sleep(60)   # onboarded + night/Friday -> stay closed
            continue
        try:
...
async def scheduled_quest_loop(accounts):
    while True:
        ...
        # Friday: closed for onboarded accounts ONLY.
        # Accounts that haven't onboarded yet stay online so onboarding runs.
        if is_friday():
            if CLOSE_ON_FRIDAY:
                pending = [a for a in accounts if not a.initialized]
                print(f"[scheduler] Friday — closing {len(accounts) - len(pending)} onboarded "
                      f"client(s); {len(pending)} pending onboarding stay online to finish it")
                for a in accounts:
                    if a.initialized:
                        await a.disconnect()
        ...
        # Night: closed for onboarded accounts ONLY (pending ones keep onboarding).
        if is_night():
            pending = [a for a in accounts if not a.initialized]
            print(f"[scheduler] Night mode (BD {now.strftime('%H:%M')}) — closing "
                  f"{len(accounts) - len(pending)} onboarded client(s); "
                  f"{len(pending)} pending onboarding stay online")
            for a in accounts:
                if a.initialized:
                    await a.disconnect()
        ...
        if slot is not None:
            ...
            tasks = []
            for a in accounts:
                # pending accounts onboard via their own connection flow; skip them
                # in the slot so they don't double-run a quest
                if not a.initialized:
                    continue
                tasks.append(asyncio.create_task(a.run_quest_once(source="slot")))
                await asyncio.sleep(random.uniform(2, 6))  # stagger to soften rate limits
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for a, res in zip([x for x in accounts if x.initialized], results):
...
async def main():
    ...
    already = len(STATE.get("initialized", []))
    print(f"Loaded {len(tokens)} account(s) | {already} already onboarded (from {STATE_FILE})")
    ...
    print(f"Night close: {NIGHT_START_HOUR}:00 PM-{NIGHT_END_HOUR}:00 AM BD (onboarded clients CLOSED)")
    print(f"Friday: {'fully closed (after onboarding)' if CLOSE_ON_FRIDAY else 'online but no quests'}")
